...
def save_data(data):
    ...
    try:
        ...
        cur = conn.cursor()
        for item in data:
            city, position_name, salary_level = item
            sql = f"""INSERT INTO lagou_job (city, position_name, salary_level) VALUES ('{city}', '{position_name}', '{salary_level}')"""
            cur.execute(sql)
        cur.close()
        conn.commit()
